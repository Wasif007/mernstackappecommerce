...
export const getAdminProduct = (keywordFromParam="",currentPage=1,price=[0,25000],category) => async (dispatch) => {
    try {
      dispatch({ type: ALL_PRODUCT_REQUEST });
  let link=`/api/v1/admin/products?keyword=${keywordFromParam}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}`;
  if(category){
     link=`/api/v1/admin/products?keyword=${keywordFromParam}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&category=${category}`;
